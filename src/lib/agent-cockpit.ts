...
- cockpit_contact_manuel : membre_id, telegram (pseudo @ consolidé à la main — il PRIME sur celui de cockpit_membres_etat), telegram_num (numéro utilisateur u…, jamais collecté ailleurs), maj_le. Pour trouver QUI retirer ou contacter sur Telegram, regarde d'abord ici, puis le pseudo déclaré au paiement.
...
⚠️ Pour relier une fiche d'archive au monde des paiements, passe par membre_id (ou directement par la vue cockpit_membre_telegram), et dis ce que vaut le lien : membre_confiance porte la fiabilité du rattachement. membre_lie_par dit la clé : 'pseudo' et 'email' sont certains, 'nom' est probable — deux Mathieu et quatre Kevin dans l'archive, un nom n'est jamais une preuve. Ce pont est rempli depuis le 06/09 (37 membres) ; avant, il était vide, et c'est pour ça que le numéro utilisateur d'un membre paraissait « collecté nulle part » alors que 1 275 personnes de l'archive en portent un.
