...
export default function LegacyCanvasPage() {
  // Canvas Legacy utilise un ID fixe pour vos données de dev
  const canvasId = "legacy"
  ...
  const [selectedNoteForTakeaway, setSelectedNoteForTakeaway] = useState<string | null>(null)
  ...
  const loadData = async () => {
    try {
      ...
      // Charger les données du canvas legacy
      const [notesRes, coursesRes] = await Promise.all([
        fetch(`/api/canvas/${canvasId}/notes`).catch(() => ({ ok: false, status: 'network error' })),
        fetch(`/api/canvas/${canvasId}/courses`).catch(() => ({ ok: false, status: 'network error' }))
      ])
      ...
      try {
        const instructorsRes = await fetch(`/api/canvas/${canvasId}/instructors`)
  ...
  const updateNote = async (updates: Partial<NoteData> & { id: string }) => {
    ...
    try {
      await fetch(`/api/canvas/${canvasId}/notes/${updates.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const createCourse = () => {
    ...
    fetch(`/api/canvas/${canvasId}/courses`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        a.href = url
        a.download = `canvas-legacy-${new Date().toISOString().split('T')[0]}.pdf`
        a.click()
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="text-lg">Chargement du canvas legacy...</div>
      </div>
    )
  }
  ...
  const deleteNote = async (noteId: string) => {
    ...
    try {
      const response = await fetch(`/api/canvas/${canvasId}/notes/${noteId}`, {
        method: 'DELETE',
      })
  ...
  const handleCreateCourse = async (courseName: string, instructorId?: string) => {
    ...
    try {
      const response = await fetch(`/api/canvas/${canvasId}/courses`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(newCourse)
      })
  ...
  const handleCreateInstructor = async (instructorName: string) => {
    ...
    try {
      const response = await fetch(`/api/canvas/${canvasId}/instructors`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleAddConcept = async (noteId: string, conceptName: string, category?: string) => {
    try {
      const response = await fetch(`/api/canvas/${canvasId}/notes/${noteId}/concepts`, {
        method: 'POST',
        headers: {
  ...
  const handleRemoveConcept = async (noteId: string, conceptNameOrId: string) => {
    try {
      ...
      const response = await fetch(`/api/canvas/${canvasId}/notes/${noteId}/concepts?conceptId=${conceptId}`, {
        method: 'DELETE'
      })
  ...
  const handleSaveTakeaway = async (noteId: string, takeaway: string) => {
    try {
      const response = await fetch(`/api/canvas/${canvasId}/notes/${noteId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ mainTakeaway: takeaway })
      })

      if (!response.ok) {
        throw new Error('Erreur lors de la sauvegarde du takeaway')
      }

      // Mettre à jour la note dans l'état local
      setNotes(prev => prev.map(note => 
        note.id === noteId 
          ? { ...note, mainTakeaway: takeaway, updatedAt: new Date() }
          : note
      ))

      // Mettre à jour la note dans l'éditeur si c'est celle qui est ouverte
      if (contentEditorNote && contentEditorNote.id === noteId) {
        setContentEditorNote(prev => prev ? { ...prev, mainTakeaway: takeaway } : null)
      }

      console.log('Takeaway sauvegardé avec succès')
    } catch (error) {
  ...
  const openConceptsModal = () => {
    if (contentEditorNote) {
      setSelectedNoteForTagging(contentEditorNote.id)
      setShowTaggingModal(true)
    }
  }
  ...
  const openTakeawayModal = () => {
    if (contentEditorNote) {
      setSelectedNoteForTakeaway(contentEditorNote.id)
    ...
    >
      ...
      {contentEditorNote && (
        <BlockBasedEditor
          ...
          onOpenConcepts={openConceptsModal}
          onOpenTakeaway={openTakeawayModal}
        />
      )}
      ...
      <TakeawayModal
        ...
        note={selectedNoteForTakeaway ? notes.find(n => n.id === selectedNoteForTakeaway) || null : null}
        onClose={() => {
          setShowTakeawayModal(false)
          setSelectedNoteForTakeaway(null)
        }}
        onSave={handleSaveTakeaway}
      />

      {/* Indicateur Canvas Legacy */}
      <div className="absolute bottom-4 right-4 z-40 pointer-events-none">
        <div className="bg-gradient-to-r from-purple-600 to-pink-600 text-white px-4 py-2 rounded-lg text-sm font-medium shadow-lg">
          🧪 Canvas Legacy (Dev)
        </div>
      </div>
    </div>
