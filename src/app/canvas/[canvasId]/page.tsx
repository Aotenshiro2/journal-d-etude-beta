...
export default function CanvasPage() {
  ...
  const loadData = async () => {
    try {
      ...
      // Charger les données essentielles du canvas spécifique
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        a.href = url
        a.download = `canvas-${canvasId}-${new Date().toISOString().split('T')[0]}.pdf`
        a.click()
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="text-lg">Chargement du canvas...</div>
      </div>
    )
  }
  ...
  return (
    ...
    >
      ...
      {/* Notification et partage Canvas */}
      <CanvasNotification canvasId={canvasId} />
    </div>
  )
}